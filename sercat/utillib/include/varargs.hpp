...
constexpr decltype(auto) logical_or(const Args &... args){
    return accumulate( jab::func::logical_or_anything(), args...);
}

template<typename... Args>
constexpr decltype(auto) logical_and(const Args &... args){
    return accumulate( jab::func::logical_and_anything(), args...);
}

template<typename... Args>
constexpr decltype(auto) sum(const Args &... args){
    return accumulate( jab::func::add_anything(), args... );
}

template<typename... Args>
constexpr decltype(auto) max(const Args &... args){
    return accumulate( jab::func::max_anything(), args... );
}
